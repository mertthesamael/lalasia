    ...
    <section className='w-full flex justify-center'>
      <div className="flex flex-col py-10 md:py-28 items-center  gap-8 md:gap-24 w-full max-w-screen-xl mx-5 md:mx-20 ">
        ...
        <div className='w-full  flex flex-wrap gap-2'>
          {articleFilters.map((el, _i) => <FilterButton onClick={() => setActiveFilter(el)} key={_i} activeFilter={activeFilter} text={el}/>)}
        </div>
        <div className='w-full flex flex-col gap-7'>
          {trendingArticles.map((el, _i) => <ArticleCard key={_i} horizontal responsive id={el.id} author={el.author} title={el.title} content={el.content} category={el.category} date={el.date} imgUrl={el.imgUrl}/>)}
        </div>
        <div>
          <PrimaryButton text='Load More' className='bg-white border font-bold !text-black' />
        </div>
      </div>
    </section>
  )
