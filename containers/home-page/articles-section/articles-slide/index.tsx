...
const ArticlesSlide: FC<ArticlesSlideProps> = ({  }) => {
  ...
  return (
    ...
    <Swiper className={styles.articleSlide} onSwiper={(swiper) => swiperRef.current = swiper}>
      {highlightedArticles.map((el, _i) => <SwiperSlide key={_i} className={styles.articleSlide__slide}><ArticleCard id={el.id} content={el.content} imgUrl={el.imgUrl} author={el.author} title={el.title} category={el.category} date={el.date}/></SwiperSlide>)}
