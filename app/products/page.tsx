...
const getItems = async(page:number,perItem:number,sort:string|boolean) => {

    const { data } = await axios.post(getAll,{page:page,perItems:perItem,sort:sort})
    return data
...
const Products: FC<ProductsProps> = async({ searchParams }) => {
  ...
  return (
    <main>
        <ProductsHero />
        <ProductsSearch />
        <ProductsItems itemsPerView={itemPerView} products={items.data} size={items.size}/>
    </main>
  )
}
